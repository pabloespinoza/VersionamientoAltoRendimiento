...
(function ($, window, document) {
    ...
    SearchableOptionList.prototype = {
        ...
        defaults: {
            ...
            texts: {
                noItemsAvailable: 'No se encontraron datos',
            ...
            events: {
                ...
                onChange: undefined,
                onScroll: function () {
                    var selectionContainerYPos;
                    if ((navigator.userAgent.indexOf("MSIE") != -1) || (!!document.documentMode == true)) //IF IE > 10
                    {
                        selectionContainerYPos = this.$input.offset().top - this.config.scrollTarget.scrollTop() + this.$input.outerHeight(false);
                    } else {
                        if ($(".modal").is(":visible")) {
                            selectionContainerYPos = this.$input.offset().top - this.config.scrollTarget.scrollTop();
                        } else {
                            selectionContainerYPos = this.$input.offset().top - this.config.scrollTarget.scrollTop() + this.$input.outerHeight(false);
                        }
                    }
        ...
        _getActualCssPropertyValue: function ($element, property) {
            ...
            if (domElement.currentStyle) {
                return domElement.currentStyle.getAttribute(property);
